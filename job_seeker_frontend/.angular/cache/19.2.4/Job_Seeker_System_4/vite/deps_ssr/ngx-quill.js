...
var QuillService = class _QuillService {
  constructor() {
    ...
    this.quill$ = (0, import_rxjs.defer)(() => __async(this, null, function* () {
      ...
      return (0, import_rxjs.firstValueFrom)(this.registerCustomModules(this.Quill, this.config.customModules, this.config.suppressGlobalRegisterWarning));
  ...
  beforeRender(Quill, customModules, beforeRender = this.config.beforeRender) {
    ...
    if (beforeRender) {
      sources.push((0, import_rxjs.from)(beforeRender()));
    }
    return (0, import_rxjs.forkJoin)(sources).pipe((0, import_operators.map)(() => Quill));
  }
  /** @internal */
  registerCustomModules(Quill, customModules, suppressGlobalRegisterWarning) {
    if (!Array.isArray(customModules)) {
      return (0, import_rxjs.of)(Quill);
    }
    const sources = [];
    for (const customModule of customModules) {
      ...
      if ((0, import_rxjs.isObservable)(maybeImplementation)) {
        sources.push(maybeImplementation.pipe((0, import_operators.tap)((implementation) => {
    ...
    return sources.length > 0 ? (0, import_rxjs.forkJoin)(sources).pipe((0, import_operators.map)(() => Quill)) : (0, import_rxjs.of)(Quill);
  }
  static {
...
var QuillEditorBase = class _QuillEditorBase {
  ...
  ngAfterViewInit() {
    ...
    this.quillSubscription = this.service.getQuill().pipe((0, import_operators.mergeMap)((Quill) => this.service.beforeRender(Quill, this.customModules(), this.beforeRender()))).subscribe((Quill) => {
      ...
      this.zone.runOutsideAngular(() => {
        ...
        if (this.onNativeBlur.observed) {
          (0, import_rxjs.fromEvent)(this.quillEditor.scroll.domNode, "blur").pipe(takeUntilDestroyed(this.destroyRef)).subscribe(() => this.onNativeBlur.next({
            editor: this.quillEditor,
            source: "dom"
          }));
          const toolbar = this.quillEditor.getModule("toolbar");
          if (toolbar.container) {
            (0, import_rxjs.fromEvent)(toolbar.container, "mousedown").pipe(takeUntilDestroyed(this.destroyRef)).subscribe((e) => e.preventDefault());
          }
        }
        if (this.onNativeFocus.observed) {
          (0, import_rxjs.fromEvent)(this.quillEditor.scroll.domNode, "focus").pipe(takeUntilDestroyed(this.destroyRef)).subscribe(() => this.onNativeFocus.next({
            editor: this.quillEditor,
            source: "dom"
          }));
        }
